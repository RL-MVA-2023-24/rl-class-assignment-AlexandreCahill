...
class DQN(nn.Module):
    def __init__(self, env, intermediate_size, depth):
        super(DQN, self).__init__()
        self.in_layer = nn.Linear(env.observation_space.shape[0], intermediate_size)
        self.hidden_layers = nn.ModuleList([nn.Linear(intermediate_size, intermediate_size) for _ in range(depth - 1)])
        self.out_layer = nn.Linear(intermediate_size, env.action_space.n)
...
class ProjectAgent:
    def __init__(self):
        ...
        self.intermediate_size = config['intermediate_size'] if 'intermediate_size' in config.keys() else 64
        self.depth = config['depth'] if 'depth' in config.keys() else 5
        self.model = DQN(env, config['intermediate_size'], config['depth']).to(self.device)
